...
class rrPreprocessor(Preprocessor):
    ...
    def load_stat_files(self, normalization_type, str1, str2):
        ...
        file_path = os.path.join(
            self.config_data["real_data_dir"], stat_folder, mean_or_max_filename
        )
        means_or_maxs = np.load(file_path).astype("float32")
        logging.debug(f"{str1} file found, {means_or_maxs.shape}")

        file_path = os.path.join(
            self.config_data["real_data_dir"], stat_folder, std_or_min_filename
        )
    ...
    def detransform(self, data):
        ...
        if norm_type == "mean":
            ...
            if not per_pixel:
                data = ((data + 1) / 2) * (
                    self.maxs[np.newaxis, self.real_var_indices, np.newaxis, np.newaxis]
                    - self.mins[
                        np.newaxis, self.real_var_indices, np.newaxis, np.newaxis
                    ]
                ) + self.mins[np.newaxis, self.real_var_indices, np.newaxis, np.newaxis]
            else:
    ...
    def transform(self, data):
        means, stds, maxs, mins = (
            ...
            copy.deepcopy(self.stds),
            copy.deepcopy(self.maxs),
            copy.deepcopy(self.mins),
        )
        norm_type = self.normalization["type"]
        if "rr" in self.variables:
            rr_idx = self.real_var_indices[0]
        ...
        if norm_type == "mean":
            if np.ndim(stds) > 1 and "rr" in self.variables:
                if self.normalization["for_rr"]["blur_iteration"] > 0:
                    ...
                    for _ in range(self.normalization["for_rr"]["blur_iteration"]):
                        stds[rr_idx] = scipy.ndimage.convolve(
                            stds[rr_idx], gaussian_filter, mode="mirror"
                        )
            else:
            ...
            if np.ndim(maxs) > 1 and "rr" in self.variables:
                if self.normalization["for_rr"]["blur_iteration"] > 0:
                    ...
                    for _ in range(self.normalization["for_rr"]["blur_iteration"]):
                        maxs[rr_idx] = scipy.ndimage.convolve(
                            maxs[rr_idx], gaussian_filter, mode="mirror"
                        )
            else:
