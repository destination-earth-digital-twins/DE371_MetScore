...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(
                run_experiment, experiment_config, config["output_folder"], index
            )
            for index, experiment_config in enumerate(config["experiments"])
        ]
    ...
    failure_str = "\n".join(
        f"{exp_config['name']}\n Reason for failure: {exception!r}"
        for exp_config, exception, tb in failed_experiments
    )
...
def run_experiment(experiment_config, output_folder, index):
    ...
    """
    try:
        experiment_set = ExperimentSet.fromConfig(
            experiment_config, output_folder=output_folder
        )
        ...
        return (
            ...
            True,
            None,
        )  # Indicate que l'expérience a réussi, sans exception
