...
"""
# TODO : General comment : Maybe merge some functions and specify the desired type as an input parameter
...
def wasserstein_wrap(data: tuple):
    ...
    return sc.wasserstein_distance(real_data, fake_data, real_weights, fake_weights)


def W1_on_image_samples(
    ...
    for ic in range(Channel_size):
        for i_x in range(Crop_Size):
            for j_y in range(Crop_Size):
                Lists.append(
                    (
                        real_data[:, ic, i_x, j_y],
                        fake_data[:, ic, i_x, j_y],
                        real_weights,
                        fake_weights,
                    )
                )
...
def W1_center(real_data, fake_data, Crop_Size=64):
    ...
    dist = torch.tensor([0.0], dtype=torch.float32).cuda()
    ...
    def compute_W1(self, real_data, fake_data):
        ...
        if self.module == np:
            if self.channel_wise:
                dist = np.array([0.0 for var in range(Channel_size)], dtype=np.float32)
        if self.channel_wise:
