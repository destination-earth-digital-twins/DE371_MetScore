...
class ObjectsAttribution(PreprocessStandalone):
    def __init__(self, *args, **kwargs):
        super().__init__(isBatched=False, **kwargs)
        self.zone = obj.Zone(
            X_min=args[0]["lon_min"],
            Y_min=args[0]["lat_min"],
            nb_lon=args[0]["sizeW"],
            nb_lat=args[0]["sizeH"],
        )
...
class crpsMultiDates(PreprocessCondObs):
    ...
    def _calculateCore(self, processed_data):
        ...
        obs_data = processed_data["obs_data"]
        if self.isOnReal:
            return CRPS_calc.crps_multi_dates(
                obs_data, real_data, real_data, debiasing=self.debiasing
            )
        else:
            return CRPS_calc.crps_multi_dates(
                obs_data, real_data, fake_data, debiasing=self.debiasing
            )
...
class crpsDiffMultiDates(PreprocessCondObs):
    ...
    def _calculateCore(self, processed_data):
        ...
        obs_data = processed_data["obs_data"]
        if self.isOnReal:
            return CRPS_calc.crps_vs_aro_multi_dates(
                obs_data, real_data, real_data, debiasing=self.debiasing
            )
        else:
            return CRPS_calc.crps_vs_aro_multi_dates(
...
class brierSkillScore(PreprocessCondObs):
    ...
    def _calculateCore(self, processed_data):
        ...
        return BS.brier_skill_score(
            obs_data, real_data, fake_data, np.array(self.threshold)
        )
...
class skillSpreadDeviationMultidates(PreprocessCondObs):
    ...
    def _calculateCore(self, processed_data):
        ...
        obs_data = processed_data["obs_data"]
        if self.isOnReal:
            return skspd.skill_spread_deviation_multidates(
                obs_data, real_data, real_data, debiasing=self.debiasing
            )
        else:
            return skspd.skill_spread_deviation_multidates(
...
class thresholdedSkillSpreadDeviationMultidates(PreprocessCondObs):
    ...
    def _calculateCore(self, processed_data):
        ...
        obs_data = processed_data["obs_data"]
        if self.isOnReal:
            return skspd.thresholded_skill_spread_deviation_multidates(
                obs_data, real_data, real_data, debiasing=self.debiasing
            )
        else:
            return skspd.thresholded_skill_spread_deviation_multidates(
