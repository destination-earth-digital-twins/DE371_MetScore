...
def Contours(field, zone):
    ...
    if (
        np.max(field) >= 0.1
    ):  # pas besoin de faire convolve_fft si pluies pas assez fortes
        Conv_R_tot = convolve_fft(field, Kernel_tt)
        Object_RR_tt[Conv_R_tot > 0.2] = 1.0
        Object_RR_tt[Conv_R_tot <= 0.2] = 0.0

    if np.max(field) >= 2.8:
        Conv_R_moderate = convolve_fft(field, Kernel_moderate)
        Object_RR_moderate[Conv_R_moderate > 3.0] = 1.0
        Object_RR_moderate[Conv_R_moderate <= 3.0] = 0.0
    if np.max(field) >= 6.5:
        Conv_R_heavy = convolve_fft(field, Kernel_heavy)
        Object_RR_heavy[Conv_R_heavy > 7.0] = 1.0
        Object_RR_heavy[Conv_R_heavy <= 7.0] = 0.0

    return {
        "total": Object_RR_tt,
        "moderate": Object_RR_moderate,
        "heavy": Object_RR_heavy,
    }
...
def attributesCore(field, reflc, zone):
    ...
    # field contain at least 1 object
        ...
        C_mass = ndimage.measurements.center_of_mass(
            field, field_m, index=list(range(1, num + 1))
        )
        for instance in list(Liste_object_instances.keys())[
            1:
        ]:  # First instance = area without precipitation
