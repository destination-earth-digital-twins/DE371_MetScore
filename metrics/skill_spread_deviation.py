...
def skill_spread_deviation_multi_dates(
    cond, X, real_ens, debiasing=False, print_skill_aro=False
):
    """
    ...
    if debiasing:
        ...
        if print_skill_aro:
            ...
            real_ens_p[:, :, 0], real_ens_p[:, :, 1] = wc.computeWindDir(
                real_ens_p[:, :, 0], real_ens_p[:, :, 1]
            )
            ...
            logging.debug(
                "skill aro",
                np.nanmean(np.sqrt(np.nanmean(skill_aro, axis=0)), axis=(-2, -1)),
            )
    ...
    spread = X_p.var(
        axis=1, ddof=1
    )  # .std operation needs ddof=1 (delta degrees of freedom) to be unbiased

    spread_aggregated = correction_factor * np.sqrt(
        np.nanmean(spread, axis=(0, -2, -1))
    )
...
def skill_spread_vs_aro_multi_dates(cond, X, real_ens, debiasing=False):
    ...
    spread_aggregated_aro = correction_factor * np.sqrt(
        np.nanmean(spread_aro, axis=(0, -2, -1))
    )
    ...
    logging.debug(
        "spread aro",
        spread_aggregated_aro,
        "skill aro",
        skill_aggregated_aro,
        flush=True,
    )
