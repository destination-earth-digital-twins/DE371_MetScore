...
def dct2D(x):
    ...
    if x.ndim == 3:
        res = dct(
            dct(x.transpose((0, 2, 1)), norm="ortho").transpose((0, 2, 1)), norm="ortho"
        )
    else:
...
def idct2D(f):
    ...
    if f.ndim == 3:
        res = idct(
            idct(f.transpose(0, 2, 1), norm="ortho").transpose(0, 2, 1), norm="ortho"
        )
    else:
...
def fft2D(x):
    ...
    if x.ndim == 3:
        res = fft(
            fft(x.transpose((0, 2, 1)), norm="ortho").transpose((0, 2, 1)), norm="ortho"
        )
    else:
...
def PowerSpectralDensitySlow(x):
    ...
    return np.concatenate(
        (
            np.expand_dims(out, axis=-1),
            np.expand_dims(out_90, axis=-1),
            np.expand_dims(out_10, axis=-1),
        ),
        axis=-1,
    )
...
def PSD_compare_multidates(obsdata, real_data, fake_data, debiasing=False):
    ...
    real_data[:, :, 0], real_data[:, :, 1] = wc.computeWindDir(
        real_data[:, :, 0], real_data[:, :, 1]
    )
    fake_data[:, :, 0], fake_data[:, :, 1] = wc.computeWindDir(
    ...
    resh_real = real_data.reshape(
        real_data.shape[0] * real_data.shape[1], channels, H, W
    )

    resh_fake = fake_data.reshape(
...
def PowerSpectralDensity_old(x, UniRad=None, Rad=None, Inds=None):
    ...
    UniRad, Binned_Sigma, Inds, Rad = radial_bin_dct(
        dct_var(x).mean(axis=0), UniRad, Rad, Inds
    )
    return UniRad, Binned_Sigma, Inds, Rad
