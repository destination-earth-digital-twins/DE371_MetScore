...
def main():
    ...
    try:
        ...
        for metr_idx, metric in enumerate(metrics):
            logger.info(
                f"Stat test (Wilcoxon) for metric {metric['name']} being plotted"
            )
            try:
                ...
                logger.info(e)
                logger.info(
                    f"Metric {metric['name']} : One of the experiments did not have the file needed. Cannot stat test."
                )
