...
class DateDataset(Dataset):
    ...
    def init_mirror_filling(self):
        ...
        img = np.load(os.path.join("/project/home/p200177/DE_371/datasets/dataset_Meteo_France_big_domain_optim_complete","2021-06-16T21:00:00Z_complete_0_0.npy"))
        # img = np.load(file)
...
class ObsDataset(DateDataset):
    ...
    def _get_filename(self, index):
        ...
        real_hour = self.start_time + (index % (self.Lead_Times//self.dh)) * self.dh
        if "formatted_index" in format_variables:
            format_variables.remove("formatted_index")
            formatted_index = real_hour % 24
            if formatted_index < 10:
                formatted_index = "0" + str(formatted_index)
        ...
        if "date" in format_variables:
            ...
            date_0 = datetime.strptime(date, "%Y-%m-%dT%H:%M:%SZ")
    ...
    def _load_file(self, file_path):
        print("fichier chargé obs dataset : ", file_path)
...
class FakeDataset(DateDataset):
    ...
    def _get_filename(self, index):
        ...
        if "formatted_index" in format_variables:
            format_variables.remove("formatted_index")
            formatted_index = ((index * self.dh) % self.Lead_Times) 
            kwargs = {"formatted_index": formatted_index}
    ...
    def _load_file(self, file_path):
        nb_vars = len(self.config_data["dataloaders"]["fake_dataset_config"]["variables"])
        ...
        crop = self.config_data["dataloaders"]["real_dataset_config"]["crop_indices"]
        file = file[:,:,crop[0]:crop[1],crop[2]:crop[3]]
...
class RealDataset(DateDataset):
    def _get_filename(self, index):
        date = self.liste_dates_rep[index]
        names = self.df0[
            (self.df0["Date"] == f"{date}")
            & (self.df0["LeadTime"] == (index % self.Lead_Times ) * self.dh + 3)
        ]["Name"].to_list()
