...
class FakeDataset(DateDataset):
    ...
    def _get_filename(self, index):
        ...
        print('LISTE', formatted_index, kwargs)
    ...
    def _load_file(self, file_path):
        ...
        print('JE SUIS SHAPE',img.shape,img.ndim) #,img.shape[1],np.transpose(img[y_min:y_max, x_min:x_max,:],(2,0,1)).shape)
        if img.ndim>3 :
            print('sup trois',img.shape)
            if img.shape[2]!=self.config_data['sizeH']: # change it with sizeH in the config file to be more general 
                print('je passe la ', img.shape)
                return np.transpose(img[y_min:y_max, x_min:x_max,:],(3,1,2))
            else:
                print('JE SUIS SHAPE2',img.shape,img.shape[1])

                return img 
        else:
            print('inf 3',img.shape)
            if img.shape[2]!=self.config_data['sizeH']: # change it with sizeH in the config file to be more general 
                print('je passe la ', img.shape,np.transpose(img[y_min:y_max, x_min:x_max,:],(2,0,1)).shape)
...
class RandomDataset(Dataset):
    ...
    def _load_file(self, file_path): # RAJOUTER LA CONDITION SI ON EST DANS REAL OU FAKE DATASET 
        ...
        print('JE SUIS SHAPE',img.shape,img.shape[1])
        if img.shape[1]!=self.config_data['sizeH']: # change it with sizeH in the config file to be more general 
            return np.transpose(img[y_min:y_max, x_min:x_max,:],(2,0,1))
        else:
            return img 
        print('JE SUIS SHAPE2',img.shape,img.shape[1])
