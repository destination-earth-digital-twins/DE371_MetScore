...
class DateDataloader(DataLoader):
    ...
    def __next__(self):
        if self.current_index < self._data_length:
            try:
                fake_samples = np.array(self.fake_dataset[ self.current_index])
                obs_samples = np.array(self.obs_dataset[self.current_index])
                real_samples = np.array(self.real_dataset[self.current_index])
    ...
    def __next__(self):
        if self.current_index < self._data_length:
            fake_samples = np.array(
                self.fake_dataset[self.current_index])
            real_samples = np.array(
                self.real_dataset[self.current_index])
            self.current_index += min(1, self._data_length - self.current_index)
            return fake_samples, real_samples, None
        else:
            raise StopIteration
...
class ModDataloader(DateDataloader):
    ...
    def __next__(self):
        if self.current_index < self._data_length:
            try:
                fake_samples = np.array(self.fake_dataset[self.current_index]["fake"])
                mod_samples = np.array(self.fake_dataset[ self.current_index]["mod"])
                real_samples = np.array(self.real_dataset[self.current_index])
                obs_samples = np.array([self.obs_dataset[self.current_index]])
                ...
                mod_bias = mod_samples.mean(axis=(0,-2,-1)) - real_samples[:,self.real_dataset.var_indices,:,:].mean(axis=(0,-2,-1)) if self.constant_debias \
                            else mod_samples.mean(axis=0) - real_samples[:,self.real_dataset.var_indices,:,:].mean(axis=0)
                if self.constant_debias:
                    logging.debug(f"applying bias {mod_bias}")
                fake_samples = fake_samples - mod_bias[np.newaxis,:,np.newaxis,np.newaxis] if self.constant_debias else fake_samples - mod_bias[np.newaxis]
