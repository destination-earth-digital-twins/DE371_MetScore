...
class ExperimentSet(Configurable):
    ...
    def _init_experiment(self):
        ...
        data_metrics_config = {
            "var_indices": self.config_data["dataloaders"]["fake_dataset_config"][
                "var_indices"
            ],
            "real_var_indices": self.config_data["dataloaders"]["real_dataset_config"][
                "var_indices"
            ],
            "obs_var_indices": self.config_data["dataloaders"]["obs_dataset_config"][
                "var_indices"
            ],
        }
        ...
        self.metrics = [
            Metric.from_typed_config(
                metric | metrics_config["args"] | data_metrics_config
            )
        ...
        self.not_batched_metrics = [
            metric for metric in self.metrics if not metric.isBatched
        ]
    ...
    def _prep_folder(self):
        ...
        if not os.path.exists(self.current_path):
            ...
            logging.warning(
                f"Folder {self.current_path} already exists. Checking for existing config file."
            )
            try:
                ...
                raise FileNotFoundError(
                    f"Folder {self.current_path} already exists, but no config file found."
            ...
            (
                existing_config_data["metrics"]["metrics_list"],
                self.config_data["metrics"]["metrics_list"],
            ) = (None, None)

            if existing_config_data != self.config_data:
                raise ValueError(
                    f"Folder {self.current_path} already exists, but config files do not match."
    ...
    def run(self, index):
        ...
        for batch_fake, batch_real, batch_obs in tqdm(
            ...
            if not (
                (batch_fake is None) and (batch_real is None) and (batch_obs is None)
            ):
        ...
        for metric_name, results in tqdm(
            batched_metric_results.items(), desc=f"{self.name}: Saving batched results"
        ):
        ...
        if self.not_batched_metrics:
            ...
            for metric in tqdm(
                ...
                if isinstance(results, dict):
                    ...
                    results_np = np.array(results, dtype=np.float32)
                    np.save(
                        os.path.join(self.current_path, metric.name) + ".npy",
                        results_np,
                    )
