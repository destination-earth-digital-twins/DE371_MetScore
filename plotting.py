...
def main():
    ...
    try:
        ...
        assert (
            "output_plots" in config
        ), f"output_plots path must be specified in {args.config}"
        ...
        for i in range(var_number):
            ...
            for k in range(len_tests):
                ...
                print(
                    mean_bias_LT[k, :, :, i].shape,
                )

                plt.plot(
                    np.nanmean(mean_bias_LT[k, :, :, i], axis=(0, 2, 3)),
                    label=cases_clean[k],
                    color=color_p[k],
                    linestyle=line[k],
                )
        ...
        for i in range(len_tests):
            ...
            crps = np.load(
                output_folder + "/" + experiment_list[i] + "/" + names_o_met[1] + ".npy"
            )
        ...
        for i in range(var_number):
            ...
            for k in range(len_tests):
                ...
                plt.plot(
                    np.nanmean(crps_scores_LT[k, :, :, i], axis=(0)),
                    label=cases_clean[k],
                    color=color_p[k],
                    linestyle=line[k],
                )
        ...
        for i in range(len_tests):
            s_p_scores[i] = np.load(
                output_folder + "/" + experiment_list[i] + "/" + names_o_met[2] + ".npy"
            )
        ...
        for i in range(var_number):
            ...
            for k in range(len_tests):
                ...
                plt.plot(
                    np.sqrt(
                        np.nanmean(s_p_scores_LT[k, :, :, 0, i] ** 2.0, axis=(0, 2, 3))
                    ),
                    label=cases_clean[k],
                    color=color_p[k],
                    linestyle=line[k],
                ...
                plt.plot(
                    np.nanmean(
                        np.sqrt(np.nanmean(s_p_scores_LT[k, :, :, 1, i], axis=(0))),
                        axis=(-2, -1),
                    ),
                    label=cases_clean[k],
                    color=color_p[k],
        ...
        for i in range(len_tests):
            ...
            Brier_scores[i] = np.load(
                output_folder + "/" + experiment_list[i] + "/" + names_o_met[3] + ".npy"
            )
        ...
        for j in range(var_number):
            for k in range(len_tests):
                ...
                for i in range(6):
                    brier_diff[i] = np.nanmean(
                        Brier_scores_LT[0, :, :, i, j] - Brier_scores_LT[k, :, :, i, j]
                    )
                plt.plot(
                    brier_diff,
                    label=cases_clean[k],
