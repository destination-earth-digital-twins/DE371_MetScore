...
def plot_mean(
    ...
    fig, ax = plt.subplots(
        figsize=(15, 5 * len(var_names) - 5), nrows=3, ncols=len(var_names) - 1
    )
    for id, var in enumerate(var_names):
        if id > 0:
                ...
                vmin = np.min(
                    [
                        np.min(
                            np.mean(
                                packsample[
                                    :, var_id, crop[0] : crop[1], crop[2] : crop[3]
                                ]
                            )
                        )
                    ],
            ...
            im = ax[0][id - 1].imshow(
                np.mean(
                    packsample[:, var_id, crop[0] : crop[1], crop[2] : crop[3]], axis=0
                ),
                origin="lower",
                cmap=colormap_var[id - 1],
                clim=clim_mean,
            )
            ...
            im = ax[1][id - 1].imshow(
                np.mean(
                    pert_sample[:, var_id, crop[0] : crop[1], crop[2] : crop[3]], axis=0
                ),
                origin="lower",
                cmap=colormap_var[id - 1],
                clim=clim_mean,
...
def plot_samples(
    ...
    if pert_sample is not None:
        fig, ax = plt.subplots(
            figsize=(15, 5 * (len(var_names) - 1)), nrows=3, ncols=(len(var_names) - 1)
        )
        for id, var in enumerate(var_names):
            if id > 0:
                var_id = dict_var[var]
                ...
                im = ax[0][id - 1].imshow(
                    packsample[mem_idx, var_id, crop[0] : crop[1], crop[2] : crop[3]],
                    origin="lower",
                ...
                im = ax[1][id - 1].imshow(
                    pert_sample[
                        mem_pert_idx, var_id, crop[0] : crop[1], crop[2] : crop[3]
                    ],
                    origin="lower",
                    cmap=colormap_var[id],
                    clim=clim,
                ...
                im = ax[2][id - 1].imshow(
                    diff, origin="lower", cmap="RdYlGn", clim=(-2, 2)
                )
                fig.colorbar(im, ax=ax[2][id - 1], shrink=0.5)
    else:
        fig, ax = plt.subplots(
            figsize=(15, 5 * (len(var_names) - 1)), nrows=1, ncols=(len(var_names) - 1)
        )
        for id, var in enumerate(var_names):
            if id > 0:
                var_id = dict_var[var]
                ax[id - 1].set_title(f"{axis_title_global}{var} real")
                im = ax[id - 1].imshow(
                    packsample[mem_idx, var_id, crop[0] : crop[1], crop[2] : crop[3]],
                    origin="lower",
                    cmap=colormap_var[id],
                )
                fig.colorbar(im, ax=ax[id - 1], shrink=0.5)
...
def plot_obs_data_on_samples(
    ...
    im = ax.imshow(
        packsample[mem_idx, 2, crop[0] : crop[1], crop[2] : crop[3]],
        origin="lower",
        cmap="coolwarm",
    )
    fig.colorbar(im, ax=ax, shrink=0.5)
    ax.imshow(
        obs_data_mask[2, crop[0] : crop[1], crop[2] : crop[3]],
        origin="lower",
        alpha=0.5,
    )
...
def plot_quantiles(
    ...
    quantiles_arome = np.quantile(
        packsample[:, :, crop[0] : crop[1], crop[2] : crop[3]], quantiles_list, axis=0
    )
    quantiles_gen = np.quantile(
        pert_sample[:, :, crop[0] : crop[1], crop[2] : crop[3]], quantiles_list, axis=0
    )
    ...
    # Quantiles of AROME
    fig, ax = plt.subplots(
        figsize=(5 * len(quantiles_list), 15),
        nrows=len(var_names) - 1,
        ncols=len(quantiles_list),
    )
    for quantile_idx, quantile in enumerate(quantiles_list):
        for var_idx, var in enumerate(var_names):
            if var_idx > 0:
                ...
                clim = (
                    vmins[quantile_idx][var_idx - 1],
                    vmaxs[quantile_idx][var_idx - 1],
                )

                ax[var_idx - 1][quantile_idx].set_title(f"{var} real - Q{quantile}")
                im = ax[var_idx - 1][quantile_idx].imshow(
                    quantiles_arome[quantile_idx][var_idx],
                    origin="lower",
                    cmap=cmap,
                    clim=clim,
                )
    ...
    for quantile_idx, quantile in enumerate(quantiles_list):
        for var_idx, var in enumerate(var_names):
            if var_idx > 0:
                ...
                im = ax[var_idx - 1][quantile_idx].imshow(
                    quantiles_gen[quantile_idx][var_idx],
                    origin="lower",
                    cmap=cmap,
                    clim=clim,
                )
...
def plot_quantiles_comparison(
    ...
    packsample_ff = np.sqrt(
        packsample[:, dict_var["u"], crop[0] : crop[1], crop[2] : crop[3]] ** 2
        + packsample[:, dict_var["v"], crop[0] : crop[1], crop[2] : crop[3]] ** 2
    )
    quantiles_arome_ff = np.quantile(packsample_ff, quantiles_list, axis=0)
    pert_sample_ff = np.sqrt(
        pert_sample[:, dict_var["u"], crop[0] : crop[1], crop[2] : crop[3]] ** 2
        + pert_sample[:, dict_var["v"], crop[0] : crop[1], crop[2] : crop[3]] ** 2
    )
    quantiles_gen_ff = np.quantile(pert_sample_ff, quantiles_list, axis=0)
    quantiles_diff_ff = np.zeros((len(quantiles_list)))

    quantiles_arome_t2m = np.quantile(
        packsample[:, dict_var["t2m"], crop[0] : crop[1], crop[2] : crop[3]],
        quantiles_list,
        axis=0,
    )
    quantiles_gen_t2m = np.quantile(
        pert_sample[:, dict_var["t2m"], crop[0] : crop[1], crop[2] : crop[3]],
        quantiles_list,
        axis=0,
    )
    ...
    for q in range(len(quantiles_list)):
        ...
        quantiles_diff_t2m[q] = (
            quantiles_gen_t2m[q].mean() - quantiles_arome_t2m[q].mean()
        )
    ...
    ax[0].plot(
        range(len(quantiles_list)),
        np.zeros(np.size(quantiles_list)),
        linewidth=5,
        c="b",
    )
...
def plot_probability_exceeding_threshold_wind(
    ...
    wind_arome = np.sqrt(
        packsample[:, dict_var["u"]] ** 2 + packsample[:, dict_var["v"]] ** 2
    )
    wind_gen = np.sqrt(
        pert_sample[:, dict_var["u"]] ** 2 + pert_sample[:, dict_var["v"]] ** 2
    )
    fig, ax = plt.subplots(
        figsize=(10 * len(threshold_wind), 15), ncols=len(threshold_wind), nrows=2
    )
    ...
    for threshold_id, threshold in enumerate(threshold_wind):
        ...
        if wind_obs_data is not None:
            for x, y in zip(coords_obs[0], coords_obs[1]):
                # ax[0][threshold_id].imshow(np.where(wind_obs_data>=threshold,1,0),
                # origin="lower", cmap='Reds', alpha=0.5)
                # ax[1][threshold_id].imshow(np.where(wind_obs_data>=threshold,1,0),
                # origin="lower", cmap='Reds', alpha=0.5)
                ax[0][threshold_id].text(
                    y, x, round(wind_obs_data[x, y] * 3.6, 2), alpha=0.5
                )
                ax[1][threshold_id].text(
                    y, x, round(wind_obs_data[x, y] * 3.6, 2), alpha=0.5
                )

    fig.suptitle(
        f"Probability Exceeding Wind Threshold map comparison for " + title_info
    )
    fig.tight_layout()
...
def plot_probability_exceeding_threshold_temperature(
    ...
    fig, ax = plt.subplots(
        figsize=(10 * len(threshold_t2m), 15), ncols=len(threshold_t2m), nrows=2
    )
    for threshold_id, threshold in enumerate(threshold_t2m):
        ...
        ax[0][threshold_id].set_title(
            f"AROME {len(packsample)} members - P(T>{threshold-273.15}°C)"
        )
        fig.colorbar(im, ax=ax[0][threshold_id], shrink=0.5)
        ...
        ax[1][threshold_id].set_title(
            f"Generated {len(pert_sample)} members - P(T>{threshold-273.15}°C)"
        )
        fig.colorbar(im, ax=ax[1][threshold_id], shrink=0.5)
        if obs_data is not None:
            for x, y in zip(coords_obs[0], coords_obs[1]):
                ax[0][threshold_id].text(
                    y, x, round(obs_data[x, y] - 273.15, 2), alpha=0.5
                )
                ax[1][threshold_id].text(
                    y, x, round(obs_data[x, y] - 273.15, 2), alpha=0.5
                )
    ...
    fig.suptitle(
        f"Probability Exceeding Temperature Threshold map comparison for " + title_info
    )
    fig.tight_layout()
    fig.savefig(figname_info + ".png", dpi=100)
    plt.close()
...
def compute_density(
    density, colors=["white", "gold", "orange", "red", "magenta", "purple"]
):
    return np.where(
        density < 0.01,
        colors[0],
            ...
            np.where(
                ...
                np.where(
                    ...
                    np.where(
                        density < 0.75,
                        colors[4],
                        np.where(density < 1.0, colors[5], "black"),
                    ),
                ),
            ),
        ),
...
def plot_panache_density_dynamic(
    ...
    arome = packsample[:, :, :, coord[1], coord[0]]
    gen = pert_sample[:, :, :, coord[1], coord[0]]
    if obs_data is not None:
        observation = obs_data[:, :, coord[1], coord[0]]
    ...
    colors = ["green", "gold", "orange", "red", "magenta", "purple"]
    # colors=['purple', 'magenta', 'red', 'orange', 'gold','white']
    cmap = mpl.colors.ListedColormap(colors)
    norm = mpl.colors.BoundaryNorm(threshold_density, cmap.N)

    for id_var, var_name in enumerate(var_names):
        if id_var > 0:
            levels_arome = np.arange(
                minimum_arome[id_var], maximum_arome[id_var], dL_arome[id_var]
            )
            levels_gen = np.arange(
                minimum_gen[id_var], maximum_gen[id_var], dL_gen[id_var]
            )
            ...
            for id_lt in range(lt_arome):
                ...
                for level_id in range(len(levels_arome) - 1):
                    ...
                    density = np.mean(
                        np.multiply(
                            arome[id_lt, :, id_var] >= borne_inf_level,
                            arome[id_lt, :, id_var] <= borne_sup_level,
                        )
                    )
                    color = compute_density(density, colors=colors)
                    axes[id_var - 1][0].scatter(
                        id_lt * 3,
                        borne_inf_level + (borne_sup_level - borne_inf_level) / 2,
                        color=str(color),
                        linewidth=10,
                    )
                    ...
                    density = np.mean(
                        np.multiply(
                            gen[id_lt, :, id_var] >= borne_inf_level,
                            gen[id_lt, :, id_var] <= borne_sup_level,
                        )
                    )
                    color = compute_density(density, colors=colors)
                    axes[id_var - 1][2].scatter(
                        id_lt * 3,
                        borne_inf_level + (borne_sup_level - borne_inf_level) / 2,
                        color=str(color),
                        linewidth=10,
            ...
            axes[id_var - 1][0].plot(
                list(range(0, lt_arome * 3, 3)), min_enveloppe_arome_lt, color="black"
            )
            axes[id_var - 1][0].plot(
                list(range(0, lt_arome * 3, 3)), max_enveloppe_arome_lt, color="black"
            )
...
def plot_panache_density_dynamic_per_variable(
    packsample,
    pert_sample,
    crop=[0, -1, 0, -1],
    title_info=" ",
    ...
    arome = packsample[:, :, :, coord[1], coord[0]]
    print("arome shape", arome.shape)
    gen = pert_sample[:, :, :, coord[1], coord[0]]
    print("gen shape", gen.shape)
    gen = np.concatenate(
        (
            pert_sample[:, :, :, coord[1], coord[0]],
            packsample[:, :, :, coord[1], coord[0]],
        ),
        axis=1,
    )
    ...
    for id_var, var_name in enumerate(var_names):
        fig, ax = plt.subplots(figsize=(25, 5), ncols=3, nrows=1, layout="constrained")
        ax[0].set_title("Panache AROME")
        ax[1].set_title("Panache AROME&Generated")
        ax[2].set_title("Panache Generated")
        threshold_density = [0, 0.05, 0.25, 0.50, 0.75, 1]
        colors = ["lightgray", "gold", "orange", "red", "magenta", "purple"]
        # colors=['purple', 'magenta', 'red', 'orange', 'gold','white']
        cmap = mpl.colors.ListedColormap(colors)
        norm = mpl.colors.BoundaryNorm(threshold_density, cmap.N)
        if id_var > 0:
            ...
            print("levels_arome", levels_arome.shape)
            levels_gen = np.arange(
                minimum_gen[id_var], maximum_gen[id_var], dL_gen[id_var]
            )
            print("levels_gen", levels_gen.shape)
            minimum_global = np.min((minimum_arome[id_var], minimum_gen[id_var]))
            maximum_global = np.max((maximum_arome[id_var], maximum_gen[id_var]))

            ax[0].set_xlabel("leadtimes (h)")
            ax[0].set_ylim([minimum_global, maximum_global])

            ax[1].set_xlabel("leadtimes (h)")
            ax[1].set_ylim([minimum_global, maximum_global])

            ax[2].set_xlabel("leadtimes (h)")
            ax[2].set_ylim([minimum_global, maximum_global])
            ...
            fig.suptitle(f"Plumes comparison of {var_name} for " + title_info)
            fig.savefig(figname_info + f"_{coord}_{dict_var[id_var]}.pdf", dpi=100)
            plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gen_data_dir",
        type=str,
        default="/project/scratch/p200177/DE_371/maxencelame/results/samples_conditioned/test_1_cond_var_norm/",
    )
    parser.add_argument(
        "--obs_dir",
        type=str,
        default="/project/home/p200177/DE_371/datasets/dataset_Meteo_France/obs_databases/",
    )
    ...
    parser.add_argument("--date_start", type=str, default="2021-08-10")
    parser.add_argument("--date_stop", type=str, default="2021-08-18")
    parser.add_argument(
        "--leadtimes",
        type=str2intlist,
    ...
    output_dir = (
        params.gen_data_dir
        + f"subjective_scores_bis_/{params.date_start}_to_{params.date_stop}/"
    )
    ...
    for date_ in list_dates:
        ...
        for id_lt, lt in enumerate(params.leadtimes):
            ...
            Ens_Gen = np.load(
                f"{params.gen_data_dir}4var_fake_ensemble_{datename}_{lt}.npy"
            )
            ...
            if lt >= 24:
                ...
                print(obs_date_name)
            obs_date_filename = f"obs{obs_date_name}_{(lt-3)%24}.npy"
            print(obs_date_filename)
            obs_data = obs_clean(
                np.load(params.obs_dir + obs_date_filename).astype(np.float32),
                [180, 436, 500, 756],
            )
            ...
            #         var_names=['rr', 'u','v','t2m'],
            #         dict_var={'rr': 0, 'u': 1, 'v': 2, 't2m': 3},
            #         threshold_wind=[10/3.6,20/3.6,30/3.6,40/3.6],
        ...
        directory = output_dir + "/panaches_per_variable/"
        if not os.path.exists(directory):
            os.makedirs(directory)
        for coord in coord_list:
            plot_panache_density_dynamic_per_variable(
                np.array(Ens_AROME_date),
                np.array(Ens_Gen_date),
                title_info=f"{datename}",
                figname_info=directory + f"panache_{datename}_{coord}",
                coord=coord,
