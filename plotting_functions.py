    ...
    for var_idx in range(config['var_number']):
        ...
        for exp_idx,exp in enumerate(experiments):
            if exp_idx>0:
                ...
                print('LA VALEUR1',np.nanmean(mean_bias_LT[exp_idx,:,:,var_idx], axis = (0,2,3)).shape,np.nanmean(mean_bias_LT[exp_idx,:,:,var_idx], axis = (0,2,3)))
                ...
                print('LA VALEUR2',np.nanmean(mean_bias_LT[exp_idx,:,:,var_idx], axis = (0,2,3)).shape,np.nanmean(mean_bias_LT[exp_idx,:,:,var_idx], axis = (0,2,3)))
                plt.plot(np.nanmean(mean_bias_LT[exp_idx,:,:,var_idx], axis = (0,2,3)),
...
def plot_ensembleCRPS(experiments, metric, config):
    ...
    for var_idx in range(config['var_number']):
        ...
        plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' + metric['name']+ '_diff_'+ case_name_thresholds[var_idx]+'_'+ exp['name']+'.png')
        plt.close()
    ...
    for var_idx in range(config['var_number']):
        ...
        plt.legend(fontsize=10, ncol=1, frameon=False, loc='lower right')
        plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' + metric['name'] + case_name_thresholds[var_idx] +'.png')
        plt.close()
    del crps_scores
    del crps_scores_LT
    gc.collect()
...
def plot_ensembleCRPSunfair(experiments, metric, config):
    crps_scores = np.zeros((len(experiments), config['number_dates'] * config['lead_times'], config['var_number']), dtype = ('float32'))
    crps_scores_LT = np.zeros((len(experiments), config['number_dates'],config['lead_times'], config['var_number']), dtype = ('float32'))
    print(f"Number of dates: {config['number_dates']}, Lead times: {config['lead_times']}")
...
def plot_skillSpread(experiments, metric, config):
    ...
    print('SHAPE spscoreslt',s_p_scores_LT.shape)
    for exp_idx, exp in enumerate(experiments):
        print('SHAPE s_p_scores',s_p_scores.shape,exp_idx)
...
def plot_brierScore(experiments, metric, config):
    ...
    for var_idx in range(config['var_number']):
        ...
        plt.legend(fontsize = 10, ncol=1, frameon = False, loc='lower right')
        plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' + metric['name'] + '_diff_thresholds_' + case_name_thresholds[var_idx] + '.png')
        plt.close()
...
def plot_rankHistogram(experiments, metric, config):
    ...
    """for var_idx in range(config['var_number']):
        ...
        plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' + metric['name'] + '_' + case_name_thresholds[var_idx] + f'_compar_{comp_name}.png')
    ...
    for exp_idx, exp in enumerate(experiments):
        for var_idx in range(config['var_number']):
            ...
            axs.tick_params(length=12, width=1)
            plt.yticks(fontsize ='16')
            plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' + metric['name'] + '_' + case_name_thresholds[var_idx] + '_'+ exp['short_name']+'.png')
    ...
    for exp_idx, exp in enumerate(experiments):
        for var_idx in range(config['var_number']):
            ...
            axs.tick_params(length=12, width=1)
            plt.yticks(fontsize ='16')
            plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' + metric['name'] + '_' + case_name_thresholds[var_idx] + '_'+ exp['short_name']+'_wo_outliers.png')
    ...
    for exp_idx, exp in enumerate(experiments):
        for var_idx in range(config['var_number']):
            ...
            axs.tick_params(length=12, width=1)
            plt.yticks(fontsize ='16')
            plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' + metric['name'] + '_' + case_name_thresholds[var_idx] + '_'+ exp['short_name']+'_renorm.png')
...
def plot_relDiagram(experiments, metric, config):
    ...
    for threshold in range(6):
        for var_idx in range(config['var_number']):
            ...
            plt.legend(fontsize = 10, ncol=1, frameon = False, loc='lower right')
            plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' + metric['name'] +'_' + str(threshold) + '_' + case_name_thresholds[var_idx] +'.png')
...
def plot_ROC(experiments, metric, config):
    ...
    for var_idx in range(config['var_number']):
        ...
        plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' + 'AROCall_' + case_name_thresholds[var_idx] +'.png')
        plt.close()
...
def plot_spectralCompute(experiments, metric, config):
    ...
    for var_idx in range(config['var_number']):
        print('JE SUIS VAR INDICE', var_idx)
        fig,axs = plt.subplots(figsize = (9,7))
        for exp_idx, exp in enumerate(experiments):
        ...
        plt.yscale("log")
        plt.legend(fontsize = 14,frameon = False, ncol=1)
        plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' +  metric['name'] + '_' + base_vars[var_idx] +'.png')  
...
def plot_SWD(experiments, metric, config):
    ...
    plt.yscale("log")
    plt.legend(fontsize = 14,frameon = False, ncol=1)
    plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' +  metric['name'] +'.png')
...
def plot_AreaProportion(experiments, metric, config):
    # Définition des seuils pour les tracés
    threshs = [0, 1, 3, 5, 10, 15, 20, 25, 30]#, 40, 50]
    plt.figure()
    # Charger et tracer les données pour chaque expérience
    for exp in experiments:
        # Construire le chemin vers les fichiers de données
        data_files = config['expe_folder'] + '/' + exp['name'] +'/'+  'AreaProportion.npy'
        
        # Charger les données pour l'expérience
        datas = np.load(data_files)
        
        # Tracer les données pour chaque expérience avec leur short_name comme légende
    ...
    # Ajouter la légende pour toutes les expériences
    plt.legend()

    # Sauvegarder le graphique dans le chemin de sortie spécifié
    output_path = os.path.join(config['output_plots'], metric['folder'],metric['name']  + '.png')
    output_path_pdf = os.path.join(config['output_plots'], metric['folder'],metric['name']  + '.pdf')
...
def plot_MultivarCorr(experiments, metric, config):
    ...
    for exp_idx, exp in enumerate(experiments):
        if not "AROME" in exp['name']:
            ...
            plt.savefig(config['output_plots'] + '/' + metric['folder'] + '/' +  metric['name'] + exp['name'] + '.png')
            plt.close()
