...
cmapRaf = colors.ListedColormap(['#ffffff', '#00ffff', '#009bff', '#007cff', '#0000ff', '#b200ff', '#ff00ff', '#ff8700', '#e55b00', '#cc2d00', '#b20000', '#720000'])
boundsRaf = [ 0.0, 
    20.0, 
    30.0, 
    40.0, 
...
def get_boundaries(Zone) :
    ...
    """
    Zone_l=["NO","SO","SE","NE","C","SE_for_GAN", "SE_GAN_extend", "MassifCentral"]
...
class canvasHolder():
    ...
    def plot_data_ff_t2m(self, data, var_names,
                       plot_dir, pic_name,
                       contrast=False,
                       withQuiver=True):
        """
        ...
        for ind in range(data2plot.shape[0]):
            ...
            for i, var in enumerate(var_names):
                ...
                if not contrast :
                    ...
                    dataplot[i,:,:],\
                    cmap=cmap,alpha=1,
                    vmin=cvalues[0][i], vmax=cvalues[1][i],
                    transform=self.proj_plot,
                    rasterized=True)
                axes[Var+str(ind)].add_feature(cfeature.COASTLINE.with_scale('10m')) # adding coastline
                axes[Var+str(ind)].add_feature(cfeature.BORDERS.with_scale('10m')) # adding borders
                ...
                if withQuiver and Var=="ff":   
                    ...
                    ims[Var + str(ind)] = axes[Var+str(ind)].quiver(
                        ...
                        u=0.1*dataplot[i,::quiSub, ::quiSub] * direction[0].squeeze()[ind,::quiSub,::quiSub],
                        v=0.1*dataplot[i,::quiSub, ::quiSub] * direction[1].squeeze()[ind,::quiSub,::quiSub],
                        scale=32.0, color='black',
                        transform=self.proj_plot,
                        rasterized=True,
                        width=0.008,
                        )
                ...
                if not contrast:
                    ...
                    if Var!='rr':
                        ims[Var+str(ind)]=axes[Var+str(ind)].pcolormesh(
                            ...
                            self.Coords[1],\
                            data_plot[i,:,:],\
                            cmap=cmap,alpha=1,vmin=Datamin[i], vmax=Datamax[i],
                            transform=self.proj_plot,
                            rasterized=True)
                    else:
                        ims[Var+str(ind)]=axes[Var+str(ind)].pcolormesh(
