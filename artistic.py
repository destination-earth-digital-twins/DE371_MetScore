...
# 180.0,
# 200.0 ]
boundst2m = [0.0, 4.0, 8.0, 10.0, 12.0, 14.0, 16.0, 18.0, 20.0, 22.0, 24.0, 26.0, 28.0]
...
normRaf = colors.BoundaryNorm(boundaries=boundsRaf, ncolors=len(boundsRaf) - 1)
normt2 = colors.BoundaryNorm(boundaries=boundst2m, ncolors=len(boundst2m) - 1)
normRR = colors.BoundaryNorm(boundaries=boundsRR, ncolors=len(boundsRR) - 1)


def extract_lonlat():
    # Extraction des longitudes,latitudes des pdg. A modifier si grille modifiée !
    filename = "/project/home/p200177/DE_371/resources/latitude_longitude/latlon.file"
...
def get_boundaries(Zone):
    ...
    X_min = [230, 250, 540, 460, 360, 540 + 55, 540, 500]
    Y_min = [300, 150, 120, 300, 220, 120 + 78, 120, 180]
...
def standardize_samples(
    ...
    if normalize:
        ...
        Maxs = norm_vectors[1]
    if chan_ind is None and crop_inds is None:

        print("Data is data clean")
        data_clean = data

    elif chan_ind is not None and crop_inds is None:
...
class canvasHolder:
    ...
    def project(self, padX=(10, 30), padY=(5, 5), ax=None):
        """
    ...
    def makeDomainBox(
        ...
        # grid0[0].add_patch(patches.Rectangle((self.X_min, self.Y_min),
        # 128, 128, linewidth=10,edgecolor='r', facecolor='r'))
    ...
    def plot_data_normal(
        self, data, var_names, plot_dir, pic_name, contrast=False, cvalues=None
    ):
        """
        ...
        for ind in range(data.shape[0]):
            ...
            for i, var in enumerate(var_names):
                ...
                if ind == 0 or Var == "rr":
                    ...
                    grid.cbar_axes[i].tick_params(labelsize="18")
    ...
    def plot_data_ff_t2m(
        self, data, var_names, plot_dir, pic_name, contrast=False, withQuiver=True
    ):
        """
        ...
        u, v = data[:, 0:1, :, :], data[:, 1:2, :, :]
        quiSub = u.shape[-1] // 16  # subsampling ratio for arrow drawings

        module, direction = cartesian2polar(u, v)
        print(direction[0].shape)
        # converting to km/h
        data2plot = np.concatenate([3.6 * module, data[:, 2:3, :, :] - 273.15], axis=1)
        cvalues = (data2plot.min(axis=(0, -2, -1)), data2plot.max(axis=(0, -2, -1)))
        print(cvalues)
        axes = {}
        ims = {}

        for ind in range(data2plot.shape[0]):
            dataplot = data2plot[ind]
            ...
            for i, var in enumerate(var_names):
                ...
                if withQuiver and Var == "ff":
                    axes[Var + str(ind)] = self.project(ax=grid[ind])
                    ims[Var + str(ind)] = axes[Var + str(ind)].quiver(
                        x=np.array(self.Coords[0])[::quiSub, ::quiSub],
                        y=np.array(self.Coords[1])[::quiSub, ::quiSub],
                        u=0.1
                        * dataplot[i, ::quiSub, ::quiSub]
                        * direction[0].squeeze()[ind, ::quiSub, ::quiSub],
                        v=0.1
                        * dataplot[i, ::quiSub, ::quiSub]
                        * direction[1].squeeze()[ind, ::quiSub, ::quiSub],
                        scale=32.0,
                        color="black",
                        transform=self.proj_plot,
                        rasterized=True,
